...
def app_init():
    ...
    ExlCmdlineArgs.instance().add_group_arg(
        "person", "--name", "name of person in one word", "-n", required=True
    )

    ExlCmdlineArgs.instance().add_group_arg(
        "person", "--job", "job of person (default: generic)", "-j", default="Generic"
    )

    # will exit process if fails - look for logger messages
    app = ExlBaseApp(
        app_name="my_sample_app", app_desc="This is my sample app description..."
    )
...
def main():
    try:
        ...
        logger.debug(
            "env is {} and val1 => [{}] val2 => [{}] val3 => [{}]".format(
                os.environ["EXL_ENV"], conf_val_1, conf_val_2, conf_val_3
            )
        )
        ...
        print(
            "my best kept secret is: {} : {}".format(
                secret_log_mgmt["elastic_username"], secret_log_mgmt["elastic_password"]
            )
        )
